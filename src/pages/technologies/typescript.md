---
title: TypeScript
description: All materials to learn about TypeScript. Free courses, paid courses and more!
layout: ../../layouts/MainLayout.astro
image: { src: /metaimages/typescript.png, alt: 'typescript' }
---

## Free courses

- [Learn TypeScript](https://learntypescript.dev/)
- [Academind - TypeScript Course for Beginners - Learn TypeScript from Scratch!](https://www.youtube.com/watch?v=BwuLxPH8IDs)
- [No BS TS Series by Jack Herrington](https://www.youtube.com/playlist?list=PLNqp92_EXZBJYFrpEzdO2EapvU0GOJ09n)
- [Exercism - Learn TypeScript by doing exercises](https://exercism.org/tracks/typescript)
- [Matt Pocock - TypeScript exercises for beginners](https://www.totaltypescript.com/tutorials/beginners-typescript)
- [How to Learn and Use TypeScript: A Comprehensive Beginner's Guide](https://dev.to/brianschnee/how-to-learn-and-use-typescript-a-comprehensive-beginners-guide-5l)

### Polish free courses

- [Przeprogramowani - Seria Przeprogramowani.ts](https://www.youtube.com/playlist?list=PLfE0DpqEANZ0CQ9pCGlxGKPvYb1Sj6ybV)
- [Jak zacząć programować? - Typescript - kurs podstaw 🔥 Tutorial krok po kroku!](https://www.youtube.com/watch?v=5CBZ6DymX0Y)

## Paid courses

- [Academind - Understanding TypeScript](https://pro.academind.com/p/understanding-typescript)
- [Matt Pocock - Total TypeScript (for intermediate/advanced TypeScript developers)](https://totaltypescript.com/)

### Polish paid courses

- [Kurs TS od Eduweb (Overment)](https://eduweb.pl/wyszukiwarka?query=typescript)

### Additional content

#### Core concepts

- [Matt Pocock - Should you use Types or Interfaces?](https://www.youtube.com/watch?v=zM9UPcIyyhQ)
- [Mapped types](https://www.totaltypescript.com/concepts/mapped-type)
- [Type helpers](https://www.totaltypescript.com/concepts/type-helpers)
- [How to use @ts-expect-error](https://www.totaltypescript.com/concepts/how-to-use-ts-expect-error)
- [Object types](https://www.totaltypescript.com/concepts/object-types)
- [Matt Pocock - `as const` Annotations - the most underrated TypeScript feature](https://www.youtube.com/watch?v=6M9aZzm-kEc)
- [Literal types](https://www.totaltypescript.com/concepts/literal-types)
- [The `any` type](https://www.totaltypescript.com/concepts/any-type)
- [Union types](https://www.totaltypescript.com/concepts/union-types)
- [Matt Pocock - TypeScript's weirdest type](https://www.youtube.com/watch?v=i4-QixSfCpY)
- [Matt Pocock - Infer is easier than you think](https://www.youtube.com/watch?v=hLZXJTm7TEk)
- [Matt Pocock - learn const generics](https://www.youtube.com/watch?v=hk6ZkD0Vg3w)

#### Tips compilations

- [Matt Pocock's collection of TypeScript tips](https://www.totaltypescript.com/tips)
- [Matt Pocock's list of articles](https://www.totaltypescript.com/articles)
- [TypeScript tips and Tricks with Matt Pocock](https://www.youtube.com/watch?v=hBk4nV7q6-w)
- [The `Prettify` Helper - how to make type intersections cleaner](https://www.totaltypescript.com/concepts/the-prettify-helper)
- [The `PropertyKey` Type](https://www.totaltypescript.com/concepts/propertykey-type)
- [Matt Pocock - ts-reset library (Does this library fixes TypeScript?)](https://www.youtube.com/watch?v=bUts4HJJjV4)
- [Matt Pocock - how to type URLSearchParams the best way?](https://www.youtube.com/channel/UCswG6FSbgZjbWtdf_hMLaow/community?lb=UgkxaZ1JJqPi5SpFeuKyXrfHINR7DV7plVQr)

#### Common mistakes

- [Matt Pocock - Enums considered harmful](https://www.youtube.com/watch?v=jjMbPt_H3RQ)
- [Matt Pocock - Don't use explicit return types, unless...](https://www.youtube.com/watch?v=nwSe95uFN8E)
- [Matt Pocock - This TS naming convention is weird](https://www.youtube.com/watch?v=qA65QjWCl60)
- [Matt Pocock - You're typing process.env wrong](https://www.youtube.com/watch?v=q1im-hMlKhM)
- [Matt Pocock - Don't put your types here](https://www.youtube.com/watch?v=zu-EgnbmcLY)
- [Matt Pocock - Solving the 'any' problem is harder than you think](https://www.youtube.com/watch?v=ExhQxFapKCs)
- [TypeScript Features to Avoid](https://www.executeprogram.com/blog/typescript-features-to-avoid)
- [Theo - ping.gg - You Might Be Using TypeScript Wrong... (how to use TypeScript the most effective way?)](https://www.youtube.com/watch?v=RmGHnYUqQ4k)

#### Common errors and how to fix them

- [Expression of type string can't be used to index type X](https://www.totaltypescript.com/concepts/type-string-cannot-be-used-to-index-type)
- [Option 'module' must be set to 'NodeNext'](https://www.totaltypescript.com/concepts/option-module-must-be-set-to-nodenext-when-option-moduleresolution-is-set-to-nodenext)
- [Property 'X' does not exist on type 'Y'](https://www.totaltypescript.com/concepts/property-does-not-exist-on-type)
- [Explained: An Interface Can Only Extend an Object Type ("An interface can only extend an object type or intersection of object types with statically known members.")](https://www.totaltypescript.com/concepts/an-interface-can-only-extend-an-object-type)
- [Object is of type 'unknown'](https://www.totaltypescript.com/concepts/object-is-of-type-unknown)

#### Validation

- [Matt Pocock - Learn Zod](https://www.totaltypescript.com/tutorials/zod)
- [Matt Pocock - Blazing Fast Tips: Build ANYTHING with Zod + Generics](https://www.youtube.com/watch?v=9N50YV5NHaE)
- [Jack Herrington - Fixing TypeScript's Blind spot: Runtime Typechecking](https://www.youtube.com/watch?v=rY_XqfSHock)

## Tools

- [Josh tries coding - This Makes TypeScript Errors So Much Better (Pretty TypeScript Errors extension)](https://www.youtube.com/watch?v=_9y29Cyo9uU)
- [TypeScript Error Translator (VS Code extension to understand TypeScript errors)](https://marketplace.visualstudio.com/items?itemName=mattpocock.ts-error-translator)
- Convert JS objects to TypeScript (a tricky way to do it):
  - [Transform JS object to JSON](https://transform.tools/js-object-to-json)
  - [Then convert JSON to TypeScript](https://transform.tools/json-to-typescript)
- [Matt Pocock - Automate your life with TypeScript](https://www.youtube.com/watch?v=C14MSrJGGBg)

## Others

- [Where To Put Your Types in Application Code](https://www.totaltypescript.com/where-to-put-your-types-in-application-code)
